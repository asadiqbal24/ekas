@if ($courses->count() > 0)
    ...
    <div class="oks-course-item-wrap tab-content" style="margin-bottom: 25px;" data-name="{{ $course->title }}">
        <div class="oks-dis-filter-main-div-right">
            ...
            @php
                ...
                } elseif ($applicationDeadline && $now->diffInDays($applicationDeadline, false) <= 30 && $now->gt($applicationOpen)) {
            ...
            @endphp
            <div class="oks-application-btn">
                <span class="@if ($statusMessage == 'Application Open') green @elseif($statusMessage == 'Application closing soon') orangedot @elseif($statusMessage == 'Opening Soon') orangedot @else red @endif"></span>
                <p>{{ $statusMessage }}</p>
            </div>
            <div class="@if(Request::is('user/dashboard')) oks-show-more @else oks-show-more-btn @endif" data-course-id="{{ $course->id }}" style="cursor: pointer;display: flex;justify-content: space-between;align-items: center;">
    ...
    @empty
        <div class="oks-wishlist-content">
            <p class="weight-medium">Your search criteria didn't match any courses or programmes</p>
...
@else
    <div class="oks-wishlist-content">
        <p class="weight-medium">No courses are available at the moment.</p>
        <p class="blockheading">Please check back later or explore other courses available on our platform.</p>
    </div>
@endif
